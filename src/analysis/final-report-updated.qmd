---
title: "Bolivia Pilcomayo Heavy Metals Study - with WHO/CDC/codex guidelines - Updated"
author: "River Remedy"
date: today
format:
  html:
...
# Load water data
water <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_water_2006.csv")
```
...
soil <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_soil_2006.csv")
sediment <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_sed_2006.csv")
...
veg <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_veg_2006.csv")
fish <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_fish_2006.csv")
...
veg_summary <- veg %>%
  group_by(Crop) %>%
  summarise(
    mean_pb = mean(`Pb (mg/kg)`, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  mutate(
    exceeds_limit = mean_pb > codex_lead_limit,
    risk_level = case_when(
      mean_pb <= codex_lead_limit ~ "Safe",
      mean_pb <= codex_lead_limit * 3 ~ "Moderate Risk",
      mean_pb <= codex_lead_limit * 10 ~ "High Risk",
      TRUE ~ "Critical Risk"
    )
  )

veg_lead_plot <- veg_summary %>%
  ggplot(aes(x = reorder(Crop, mean_pb), y = mean_pb, fill = risk_level)) +
  geom_col(alpha = 0.8, width = 0.7) +
  geom_hline(
  ...
  labs(
    title = "**Lead Contamination in Agricultural Crops (Mean Values)**",
    subtitle = "Average lead concentration by crop, accounting for sample size variation",
    x = "Crop Type",
    y = "Mean Lead Concentration (mg/kg)",
...
human <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_human_2006.csv")
animal <- read_csv("/Users/ktdroppa/Desktop/riverremedy/data/raw/ITA_animal_2006.csv")
...
animal_summary <- animal %>%
  group_by(Animal) %>%
  summarise(
    mean_pb = mean(`Pb (mg/dL)`, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  mutate(
    exceeds_limit = mean_pb > vet_lead_limit,
    risk_level = case_when(
      mean_pb <= vet_lead_limit ~ "Safe",
      mean_pb <= vet_lead_limit * 5 ~ "Moderate Risk",
      mean_pb <= vet_lead_limit * 20 ~ "High Risk",
      TRUE ~ "Critical Risk"
    )
  )

animal_lead_plot <- animal_summary %>%
  ggplot(aes(x = reorder(Animal, mean_pb), y = mean_pb, fill = risk_level)) +
  geom_col(alpha = 0.8, width = 0.7) +
  geom_hline(
  ...
  labs(
    title = "**Lead Exposure in Livestock Blood (Mean Values)**",
    subtitle = "Mean blood lead levels by species, accounting for unequal sample sizes",
    x = "Animal Type",
    y = "Mean Blood Lead Level (mg/dL)",
